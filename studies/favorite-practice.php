...
class Tweet implements \JsonSerializable {
	...
	public static function getTweetByTweetContent(\PDO $pdo, string $tweetContent) {
		// sanitize the description before searching; trim gets rid of spaces; filter_var limits what can be searched (not everything!)
		$tweetContent = trim($tweetContent);
		$tweetContent = filter_var($tweetContent, FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES);
//		if the search is empty, return error
		if(empty($tweetContent) === true) {
			throw(new \PDOException("tweet content is invalid"));
		}
		// create query template
		// tweetContent is what the user types in
		$query = "SELECT tweetId, tweetProfileId, tweetContent, tweetDate FROM tweet WHERE tweetContent LIKE :tweetContent";
		$statement = $pdo->prepare($query);
		// bind the tweet content to the place holder in the template
		// remember: % in sql = wildcard
		$tweetContent = "%$tweetContent%";
		$parameters = ["tweetContent" => $tweetContent];
		$statement->execute($parameters);
		// build an array of tweets
		// fetch an array limited to the number of rows
		$tweets = new \SplFixedArray($statement->rowCount());
		$statement->setFetchMode(\PDO::FETCH_ASSOC);
		// while loop: while row is assigned to statement 'fetch'
		// fetch command returns rows assigned to statement (indefinitely), until it returns false (no more data)
		while(($row = $statement->fetch()) !== false) {
			// now, use the constructor
			try {
				$tweet = new Tweet($row["tweetId"], $row["tweetProfileId"], $row["tweetContent"], $row["tweetDate"]);
				// call the array (pluralized), where [key] is the incrementing integer for each row in the array.
				// then, park the desired data in the array ($tweet)
				$tweets[$tweets->key()] = $tweet;
				// this line increments the key up 1
				$tweets->next();
	 ...
	 * @throws \TypeError when variables are not the correct data type
	 **/
	// remember, the "?" is asking if it's null or not
